from main.base import Process, RBXString
from main.classdescriptor import ClassDescriptor
from tools.offsets import Offsets
from main.bytecode import Bytecode as RBXBytecode
from tools.getrenderview import GetRenderViewFromLog
import time
...
class Instance:
    ...
    def Spoof(self, other: int):
        ...
        Process.write_longlong(self.Address + 0x8, spoofed_addy) # self address
    ...
    def set_iscorescript(self, value):
        if self.ClassName.endswith("Script"):
            bool_value = 1 if value else 0
            Process.write_long(self.Address + Offsets.module_iscore, bool_value)
    ...
    def __getattr__(self, ChildName):
        return self.FindFirstChild(name=ChildName, recursive=False)
    ...
    def __setattr__(self, name: str, value) -> None:
        ...
        if name == "Bytecode" and (
            type(value) == list and len(value) == 2) and (self.ClassName in Offsets.Bytecode):
            ...
            Process.write_longlong(
                self.Address + Offsets.Bytecode[self.ClassName],value)
        elif name == "Value" and self.ClassName.endswith("Value"):
            if self.ClassName == "BoolValue":
                Process.write_byte(self.Address + Offsets.ValueBase,int(value))
            elif self.ClassName == "NumberValue":
